...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    mkdir -p data/tr_simu_2ch_multich
    <data/tr_simu_2ch/utt2spk sed -r 's/^(.*?)_[A-H](_.*?) /\1\2 /g' | sort -u > data/tr_simu_2ch_multich/utt2spk
    <data/tr_simu_2ch/text sed -r 's/^(.*?)_[A-H](_.*?) /\1\2 /g' | sort -u > data/tr_simu_2ch_multich/text
    <data/tr_simu_2ch_multich/utt2spk utils/utt2spk_to_spk2utt.pl >data/tr_simu_2ch_multich/spk2utt

    for ch in {A..B}; do
        <data/tr_simu_2ch/wav.scp grep "_${ch}_" | sed -r 's/^(.*?)_[A-B](_.*?) /\1\2 /g' >data/tr_simu_2ch_multich/wav_ch${ch}.scp
    done
    gather-wav-scp.py data/tr_simu_2ch_multich/wav_ch*.scp > data/tr_simu_2ch_multich/wav.scp
    ...
    for setname in tr_simu_2ch ${recog_set}; do
        dump_pcm.sh --nj 64 --filetype "sound.hdf5" data/${setname}_multich
    done
