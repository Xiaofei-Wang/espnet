...
multistream_set="dev_${enhancement}_u01 dev_${enhancement}_u02 dev_${enhancement}_u03 dev_${enhancement}_u04 dev_${enhancement}_u05 dev_${enhancement}_u06"
...
if [ ${stage} -le 0 ]; then
    ...
    #eval#for dset in dev eval; do
    # u05 is missing for the dev set, this is for multi-stream recognition
    for dset in dev; do
	for mictype in u01 u02 u03 u04 u05 u06; do
...
if [ ${stage} -le 1 ]; then
    ...
    for x in ${train_set} ${recog_set} ${multistream_set}; do
...
if [ ${stage} -le 2 ]; then
    ...
    utils/create_split_dir.pl \
        /export/b{04,05,06,77}/${USER}/espnet-data/egs/chime5/asr_multistream/dump/${train_set}/delta${do_delta}/storage \
        ${feat_tr_dir}/storage
    fi
    if [[ $(hostname -f) == *.clsp.jhu.edu ]] && [ ! -d ${feat_dt_dir}/storage ]; then
    utils/create_split_dir.pl \
        /export/b{04,05,06,07}/${USER}/espnet-data/egs/chime5/asr_multistream/dump/${train_dev}/delta${do_delta}/storage \
        ${feat_dt_dir}/storage
    fi
    ...
    for rtask in ${recog_set} ${multistream_set}; do
